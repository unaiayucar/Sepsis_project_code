function [sepsis_flag, sepsis_counter] = sepsis(hadm_id, sepsis_counter, sepsis_table)
    ...
    if sepsis_counter <= height(sepsis_table)
       ...
       elseif sepsis_hadm_id < hadm_id
           % we have to increase the value of the counter until we reach the
           % desired value or, we over pass it
           while(out == 0)
              sepsis_counter = sepsis_counter + 1;
              sepsis_hadm_id = sepsis_table(sepsis_counter,2).hadm_id;
              if sepsis_hadm_id == hadm_id
